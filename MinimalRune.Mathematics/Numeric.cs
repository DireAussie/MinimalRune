...
namespace MinimalRune.Mathematics
{
    ...
    /// default values <see cref="EpsilonF"/> (for <see langword="float"/>) or 
    /// <see cref="EpsilonD"/> (for <see langword="double"/>) are used.
    /// </para>
    /// <para>
    ...
    /// floating-point numbers using the default epsilon tolerance <see cref="EpsilonF"/> or 
    /// <see cref="EpsilonD"/> (see <see cref="AreEqual(float,float)"/> or 
    /// <see cref="AreEqual(double,double)"/>) then the epsilon values are automatically scaled to
    /// match the range and resolution of the floating-point numbers. When two floating-point numbers
    /// are compared using an explicit epsilon value (see <see cref="AreEqual(float,float,float)"/> or
    /// <see cref="AreEqual(double,double,double)"/>) then this epsilon value is used directly.
    /// </para>
    /// </remarks>
