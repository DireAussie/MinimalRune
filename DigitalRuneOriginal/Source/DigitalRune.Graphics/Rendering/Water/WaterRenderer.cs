...
namespace MinimalRune.Graphics.Rendering
{
  ...
  public class WaterRenderer : SceneNodeRenderer
  {
    ...
    public override void Render(IList<SceneNode> nodes, RenderContext context, RenderOrder order)
    {
      ...
      for (int i = 0; i < numberOfNodes; i++)
      {
        ...
        else if (node.SkyboxReflection != null)
        {
          ...
          Matrix orientation = node.SkyboxReflection.PoseWorld.Orientation;
          _parameterReflectionMatrix.SetValue((Matrix)(new Matrix(orientation, Vector3.Zero) * mirrorZ));
        ...
        if (node.Flow != null)
        {
          ...
          Matrix m = Matrix.CreateScale(1 / extent.X, 1, 1 / extent.Z)
                        * Matrix.CreateTranslation(-aabb.Minimum.X, 0, -aabb.Minimum.Z)
                        * Matrix.CreateScale(1 / node.ScaleLocal.X, 1, 1 / node.ScaleLocal.Z)
    ...
    private void RenderUnderwaterGeometry(WaterNode node, CameraNode cameraNode)
    {
      ...
      if (submesh != null)
      {
        ...
        _parameterWorld.SetValue((Matrix)(
          node.PoseWorld
          * Matrix.CreateScale(node.ScaleWorld)
          * data.SubmeshMatrix));
      }
      else
      {
        ...
        var world = Matrix.CreateTranslation(position) * Matrix.CreateScale(size, size / 2.0f, size);
      ...
      else if (node.Waves == null || node.Waves.DisplacementMap == null)
      {
        ...
        if (_quadSubmesh == null)
        {
          ...
          var quadShape = new TransformedShape(new GeometricObject(
            ...
            new Pose(Matrix.CreateRotationX(-ConstantsF.PiOver2)))).GetMesh(0.001f, 4);
        ...
        Matrix world = Matrix.CreateTranslation(position) * Matrix.CreateScale(size, 1, size);
        _parameterWorld.SetValue((Matrix)world);
    ...
    private void SetFogParameters(IList<FogNode> fogNodes, CameraNode cameraNode, Vector3 lightDirection)
    {
