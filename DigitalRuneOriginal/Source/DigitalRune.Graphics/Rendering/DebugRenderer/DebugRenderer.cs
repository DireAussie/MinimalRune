...
namespace MinimalRune.Graphics.Rendering
{
  ...
  public class DebugRenderer : IDisposable
  {
      ...
      Color color, bool drawWireFrame, bool drawOverScene)
    {
      ...
      var transform = pose * Matrix.CreateScale(scale);
      ...
      Color color, bool drawWireFrame, bool drawOverScene)
    {
      ...
      if (Vector3.AreNumericallyEqual(scale, Vector3.One) && !pose.HasRotation && !pose.HasTranslation)
      {
    ...
    /// disabled).
    /// </param>
    public void DrawText(string text, Vector3 position, Vector2F relativeOrigin, Color color, bool drawOverScene)
    {
      if (!Enabled || string.IsNullOrEmpty(text))
        return;
    ...
    /// disabled).
    /// </param>
    public void DrawText(StringBuilder text, Vector3 position, Vector2F relativeOrigin, Color color, bool drawOverScene)
    {
      if (!Enabled || text == null || text.Length == 0)
        return;
    ...
    public void DrawSkeleton(SkeletonPose skeletonPose, Pose pose, Vector3 scale, float axisLength, Color color, bool drawOverScene)
    {
      ...
      var world = pose * Matrix.CreateScale(scale);
