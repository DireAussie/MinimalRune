  ...
  internal sealed class PrimitiveBatch
  {
    ...
    private void RenderSphere(IGraphicsService graphicsService, PrimitiveJob job, RenderContext context)
    {
      if (DrawWireFrame)
      {
        ...
        Matrix cameraProjection = context.CameraNode.Camera.Projection;
    ...
    private void RenderSphereOutline(float radius, ref Vector3 center, ref Pose cameraPose, ref Vector3 right, ref Color color)
    {
      ...
      Vector3 cameraToPoint = Matrix.CreateRotation(normal, α) * cameraToCenter;
      ...
      for (int i = 1; i <= NumberOfSegments; i++)
      {
        ...
        Vector3 rightRotated = Matrix.CreateRotation(forward, -β) * right;
        ...
        cameraToPoint = Matrix.CreateRotation(normal, α) * cameraToCenter;
