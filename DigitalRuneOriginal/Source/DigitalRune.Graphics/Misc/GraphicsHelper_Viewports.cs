...
namespace DigitalRune.Graphics
{
  partial class GraphicsHelper
  {
    ...
    /// </returns>
    public static Vector3 Project(this Viewport viewport, Vector3 position, Matrix projection, Matrix view, Matrix world)
    {
      Matrix worldViewProjection = projection * view * world;
    ...
    /// <see cref="Viewport.MinDepth"/> to <see cref="Viewport.MaxDepth"/> - usually [0, 1].)
    /// </returns>
    public static Vector3 Project(this Viewport viewport, Vector3 position, Matrix projection, Matrix view)
    {
    ...
    /// </returns>
    public static Vector3 Project(this Viewport viewport, Vector3 position, Matrix projection)
    {
    ...
    /// </returns>
    internal static Vector3 ProjectToViewport(this Viewport viewport, Vector3 position, Matrix projection)
    {
      // Transform position to clip space. (TransformPosition() transforms the position 
      // to clip space and performs the homogeneous divide.)
      Vector3 positionClip = projection.TransformPosition(position);
    ...
    public static Vector3 Unproject(this Viewport viewport, Vector3 position, Matrix projection, Matrix view, Matrix world)
    {
      Matrix worldViewProjection = projection * view * world;
    ...
    [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Naming", "CA1704:IdentifiersShouldBeSpelledCorrectly")]
    public static Vector3 Unproject(this Viewport viewport, Vector3 position, Matrix projection, Matrix view)
    {
    ...
    [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Naming", "CA1704:IdentifiersShouldBeSpelledCorrectly")]
    public static Vector3 Unproject(this Viewport viewport, Vector3 position, Matrix projection)
    {
      Matrix fromClipSpace = projection.Inverse;
    ...
    internal static Vector2F GetScreenSize(CameraNode cameraNode, Viewport viewport, IGeometricObject geometricObject)
    {
      ...
      Matrix proj = cameraNode.Camera.Projection;
      ...
      else
      {
        ...
        Matrix cameraOrientation = cameraPose.Orientation;
    ...
    public static float GetViewNormalizedDistance(float distance, Matrix projection)
    {
