...
namespace MinimalRune.Geometry.Collisions.Algorithms
{
  ...
  internal sealed class GjkSimplexSolver : IRecyclable
  {
    ...
    {
      Debug.Assert(Vector3.AreNumericallyEqual(pointA - pointB, w), "w is unequal to pointA - pointB.");
    ...
    {
      for (int i = 0; i < NumberOfVertices; i++)
        if (Vector3.AreNumericallyEqual(_w[i], w))
          return true;

      // w could be already removed.
      if (Vector3.AreNumericallyEqual(_lastW, w))
        return true;
