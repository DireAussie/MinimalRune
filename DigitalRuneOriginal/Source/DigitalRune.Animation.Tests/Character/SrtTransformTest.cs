...
namespace DigitalRune.Animation.Character.Tests
{
  ...
  public class SrtTransformTest
  {
    ...
    public void HasRotationTest()
    {
      ...
      srt.Rotation = Quaternion.CreateRotationX(0.000001f);
      Assert.IsFalse(srt.HasRotation);

      srt.Rotation = Quaternion.CreateRotationX(0.1f);
    ...
    {
      var m = Matrix.CreateTranslation(1, 2, 3) * Matrix.CreateRotationY(0.1f) * Matrix.CreateScale(-2, 3, 4);
      Assert.IsTrue(SrtTransform.IsValid(m));

      // Concatenating to SRTs creates a skew.
      m = Matrix.CreateRotationZ(0.1f) * Matrix.CreateScale(-2, 3, 4) * m;
    ...
    public void Multiply()
    {
      ...
      var result1 = SrtTransform.Multiply(a, b).ToMatrix();
      var result2 = a * b;
      Assert.IsTrue(Matrix.AreNumericallyEqual(result1, result2));
    }
    ...
    public void FromPose()
    {
      ...
      Assert.IsTrue(Matrix.AreNumericallyEqual(pose.Orientation, srt.Rotation.ToRotationMatrix33()));
