...
namespace DigitalRune.Animation.Tests
{
  ...
  public class KeyFrameAnimationTest
  {
    ...
    public void SamplingKeyFrames()
    {
      ...
      Assert.IsTrue(Quaternion.AreNumericallyEqual(keyFrame0.Value, animation.GetValue(TimeSpan.FromSeconds(1.0), defaultSource, defaultTarget)));
    ...
    public void ShouldReturnTheFirstOrLastKeyFrame()
    {
      ...
      // Pre loop
      Assert.IsTrue(Quaternion.AreNumericallyEqual(keyFrame0.Value, animation.GetValue(TimeSpan.FromSeconds(0.0), defaultSource, defaultTarget)));

      // Post loop
      Assert.IsTrue(Quaternion.AreNumericallyEqual(keyFrame2.Value, animation.GetValue(TimeSpan.FromSeconds(3.75), defaultSource, defaultTarget)));
    }
    ...
    public void CyclicOffsetLoopBehavior()
    {
      ...
      var expected = InterpolationHelper.Lerp(keyFrame1.Value, keyFrame2.Value, 0.25f);
      Assert.IsTrue(Quaternion.AreNumericallyEqual(expected, cycleOffset * animationClip.GetValue(TimeSpan.FromSeconds(0.25), defaultSource, defaultTarget)));

      // Post loop
      expected = cycleOffset * cycleOffset * InterpolationHelper.Lerp(keyFrame1.Value, keyFrame2.Value, 0.75f);
      Assert.IsTrue(Quaternion.AreNumericallyEqual(expected, animationClip.GetValue(TimeSpan.FromSeconds(6.75), defaultSource, defaultTarget)));
    }
