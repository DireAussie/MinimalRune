...
namespace MinimalRune.Geometry.Shapes.Tests
{
  ...
  public class RayTest
  {
    ...
    public void PositiveUniformScaling()
    {
      ...
      Assert.IsTrue(Vector3.AreNumericallyEqual(pointOnRay, ray.Origin + ray.Direction * 10 * 3.5f));
    }
    ...
    public void NegativeUniformScaling()
    {
      ...
      Assert.AreEqual(direction, ray.Direction);
      Assert.AreEqual(length, ray.Length);
      Assert.IsTrue(Vector3.AreNumericallyEqual(endPoint, ray.Origin + ray.Direction * ray.Length));
    }
    ...
    public void ToWorld()
    {
      ...
      Pose pose = new Pose(new Vector3(-5, 100, -20), Matrix.CreateRotation(new Vector3(1, 2, 3), 0.123f));
      startPoint = pose.ToWorldPosition(startPoint);
      endPoint = pose.ToWorldPosition(endPoint);
      ray.ToWorld(ref pose);

      Assert.IsTrue(Vector3.AreNumericallyEqual(startPoint, ray.Origin));
      Assert.IsTrue(Vector3.AreNumericallyEqual(endPoint, ray.Origin + ray.Direction * ray.Length));
    }
