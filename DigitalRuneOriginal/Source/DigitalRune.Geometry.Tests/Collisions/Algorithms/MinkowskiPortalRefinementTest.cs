...
namespace DigitalRune.Geometry.Collisions.Algorithms.Tests
{
  ...
  public class MinkowskiPortalRefinementTest
  {
    ...
    public void ComputeCollision()
    {
      ...
      Assert.IsTrue(Vector3.AreNumericallyEqual(new Vector3(0, 0.1f, 0.2f), set[0].PositionAWorld, 0.02f));
    }
    ...
    public void TestNormals()
    {
      ...
      Assert.IsTrue(Vector3.AreNumericallyEqual(new Vector3(-1, 0, 0), set[0].Normal, 0.001f));
      ...
      for (int i = 0; i < 100000; i++)
      {
        ...
        for (int j = 0; j < 2 && haveContact == false; j++)
        {
          ...
          for (int k = 0; k < 3 && haveContact == false; k++)
          {
            ...
            if (Vector3.AreNumericallyEqual(t0.Vertex0 * u + t0.Vertex1 * v + t0.Vertex2 * w, t1[k]))
              haveContact = true;
          }
        }
        // Test line segments.
