...
namespace DigitalRune.Geometry.Collisions.Algorithms.Tests
{
  ...
  public class TriangleMeshAlgorithmTest
  {
    ...
    public void ComputeCollision()
    {
      ...
      //Assert.IsTrue(Vector3.AreNumericallyEqual(new Vector3(0, 0, 0), set[0].PositionALocal, 0.01f));  // MPR will not return the perfect contact point.
      Assert.IsTrue(Vector3.AreNumericallyEqual(new Vector3(0, 1, 0), set[0].Normal, 0.1f));
      ...
      Assert.AreEqual(1, set.Count);
      Assert.IsTrue(Numeric.AreEqual(0.1f, set[0].PenetrationDepth, 0.001f));
      Assert.IsTrue(Vector3.AreNumericallyEqual(new Vector3(0.1f, 0, 0.1f), set[0].PositionALocal, 0.01f));
      Assert.IsTrue(Vector3.AreNumericallyEqual(new Vector3(0, 1, 0), set[0].Normal, 0.1f));
      ...
      Assert.AreEqual(1, set.Count);
      Assert.IsTrue(Numeric.AreEqual(0.1f, set[0].PenetrationDepth, 0.001f));
      Assert.IsTrue(Vector3.AreNumericallyEqual(new Vector3(0.1f, 0, 0.1f), set[0].PositionBLocal, 0.01f));
      Assert.IsTrue(Vector3.AreNumericallyEqual(new Vector3(0, -1, 0), set[0].Normal, 0.1f));
      ...
      Assert.IsTrue(Vector3.AreNumericallyEqual(new Vector3(0.2f, 0, 0.3f), set[0].PositionALocal, 0.01f));
      Assert.IsTrue(Vector3.AreNumericallyEqual(new Vector3(0, 1, 0), set[0].Normal, 0.1f));
      algo.UpdateContacts(set, 0);
